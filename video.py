...
def clean_summary_text(raw_summary_text):
    # find the content strictly between BEGINQUIZ and ENDQUIZ
    summary_content_match = re.search(r"BEGIN\s*(.*?)\s*END", raw_summary_text, re.DOTALL | re.IGNORECASE)
    if summary_content_match:
        # Strip any extra whitespace from extracted content
        return summary_content_match.group(1).strip()
    return ""



def list_generator(largestring):
    sentenceList = []
    startSentence = 0
    for i in range(len(largestring)): 
        if largestring[i] == '.':
            sentence = ''
            for j in range (startSentence, i):
               sentence += largestring[j]
...
summaryStr = generate_summary()
cleanedSummary = clean_summary_text(summaryStr)
summaryList = list_generator(cleanedSummary)
